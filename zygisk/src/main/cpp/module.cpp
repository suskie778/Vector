...
constexpr char kEmergencyDisablePath[] = "/data/adb/lspd/safety/disable";

bool emergency_disable_requested() {
    return access(kEmergencyDisablePath, F_OK) == 0;
}
...
void VectorModule::preAppSpecialize(zygisk::AppSpecializeArgs *args) {
    ...
    // This file is deliberately checked before any binder lookup, manager handling, or ART/JNI
    // setup. It is the last-resort recovery switch: deleting an APK cannot undo hooks already
    // mapped into a running process, but this prevents the next process (including system_server)
    // from receiving any new Vector hooks.
    if (emergency_disable_requested()) {
        LOGW("Emergency disable is active; skipping application injection.");
        SetAllowUnload(true);
        return;
    }
...
void VectorModule::preServerSpecialize(zygisk::ServerSpecializeArgs *args) {
    if (emergency_disable_requested()) {
        should_inject_ = false;
        LOGW("Emergency disable is active; skipping system_server injection.");
        SetAllowUnload(true);
        return;
    }
