...
في حالة هذا الحساب، صفحة Xiaomi تعرض الخطوات التالية:

1. اضغط `Send` لإرسال رمز التحقق إلى البريد المقنّع الظاهر في الصفحة.
2. افتح البريد وأدخل الرمز داخل Chromium في Colab.
3. أكمل الصفحة حتى تظهر رسالة نجاح صريحة.
4. عندها فقط ارجع إلى Colab واضغط Enter.

فتح `notificationUrl` وحده لا يكمل التحقق، والضغط على Enter قبله سيجعل
Xiaomi تنشئ رابط تحقق جديداً.
