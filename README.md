...
[![Build](https://img.shields.io/github/actions/workflow/status/JingMatrix/Vector/build.yml?branch=master&event=push&logo=github&label=Build)](https://github.com/JingMatrix/Vector/actions/workflows/build.yml?query=event%3Apush+branch%3Amaster+is%3Acompleted)
...
If a module causes a crash loop, enable Vector's pre-injection circuit breaker
from a root shell before rebooting:
...
This blocks new Vector injection into applications and `system_server`. It
cannot remove hooks already present in a running process, so a reboot or
process restart is required. See [docs/recovery.md](docs/recovery.md) for the
CLI and automatic safe-mode behavior.
...
| **Canary (CI) Builds** | [GitHub Actions](https://github.com/JingMatrix/Vector/actions/workflows/build.yml?query=branch%3Amaster) |
