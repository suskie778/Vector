...
object VectorDaemon {
  ...
  val bootAttempt = FileSystem.beginBootAttempt()
  ...
  Log.i(
      TAG,
      "Vector daemon started: lateInject=$isLateInject, proxy=$proxyServiceName, " +
          "bootAttempt=$bootAttempt, emergencyDisabled=${FileSystem.isEmergencyDisabled()}")
  ...
  // Setup IPC channel for applications by injecting DaemonService binder. In emergency safe mode
  // the native loader intentionally refuses system_server injection, so do not wait for the bridge
  // or ask Android to restart a process that we are deliberately keeping untouched.
  if (FileSystem.isEmergencyDisabled()) {
    Log.w(TAG, "Emergency safe mode is active; skipping system_server bridge setup.")
  } else {
    ...
    runCatching {
          ...
          val deathRecipient =
              object : IBinder.DeathRecipient {
                ...
                val safeModeEnabled = FileSystem.recordSystemServerFailure()
                ...
                if (safeModeEnabled) {
                  Log.e(
                      TAG,
                      "Emergency safe mode enabled; refusing further system_server retries.")
                } else {
