...
object FileSystem {
  ...
  val safetyDirPath: Path = basePath.resolve("safety")
  val emergencyDisablePath: Path = safetyDirPath.resolve("disable")
  private val bootStatePath: Path = safetyDirPath.resolve("boot.properties")
  ...
  private const val BOOT_FAILURE_LIMIT = 3
  ...
  /** Returns true when the pre-injection emergency circuit breaker is active. */
  fun isEmergencyDisabled(): Boolean = Files.exists(emergencyDisablePath)

  /**
   * Enables or clears the emergency circuit breaker. The marker is intentionally outside the
   * database and module cache so it remains available when those files are corrupt or stale.
   */
  @Synchronized
  fun setEmergencyDisabled(disabled: Boolean, reason: String = "manual"): Boolean {
    return runCatching {
          Files.createDirectories(safetyDirPath)
          if (disabled) {
            Files.writeString(
                emergencyDisablePath,
                "${Instant.now()} $reason\n",
                StandardOpenOption.CREATE,
                StandardOpenOption.TRUNCATE_EXISTING,
                StandardOpenOption.WRITE)
            Os.chmod(emergencyDisablePath.toString(), "600".toInt(8))
          } else {
            Files.deleteIfExists(emergencyDisablePath)
          }
          true
        }
        .onFailure { Log.e(TAG, "Failed to change emergency disable state", it) }
        .getOrDefault(false)
  }

  /**
   * Records a boot attempt using Android's stable boot id. Three unsuccessful attempts activate
   * safe mode, while a successful system_server handshake clears the failure state.
   */
  @Synchronized
  fun beginBootAttempt(): Int {
    return runCatching {
          Files.createDirectories(safetyDirPath)
          val properties = readSafetyProperties()
          val bootId =
              runCatching {
                    File("/proc/sys/kernel/random/boot_id").readText().trim().ifEmpty { "unknown" }
                  }
                  .getOrDefault("unknown")
          val sameBoot = properties.getProperty("boot_id") == bootId
          val attempts =
              if (sameBoot && properties.getProperty("healthy") != "true") {
                properties.getProperty("attempts", "0").toIntOrNull()?.plus(1) ?: 1
              } else {
                1
              }
          properties.setProperty("boot_id", bootId)
          properties.setProperty("attempts", attempts.toString())
          properties.setProperty("failures", "0")
          properties.setProperty("healthy", "false")
          writeSafetyProperties(properties)
          if (attempts >= BOOT_FAILURE_LIMIT) {
            setEmergencyDisabled(true, "automatic safe mode after $attempts failed boots")
          }
          attempts
        }
        .onFailure { Log.e(TAG, "Failed to record boot attempt", it) }
        .getOrDefault(0)
  }
  ...
  fun markBootHealthy() {
    runCatching {
          Files.createDirectories(safetyDirPath)
          val properties = readSafetyProperties()
          properties.setProperty("healthy", "true")
          properties.setProperty("failures", "0")
          writeSafetyProperties(properties)
        }
        .onFailure { Log.w(TAG, "Failed to mark boot healthy", it) }
  }

  /** Records a system_server failure and enables safe mode after repeated failures in one boot. */
  @Synchronized
  fun recordSystemServerFailure(): Boolean {
    if (isEmergencyDisabled()) return true
    return runCatching {
          Files.createDirectories(safetyDirPath)
          val properties = readSafetyProperties()
          val failures = properties.getProperty("failures", "0").toIntOrNull()?.plus(1) ?: 1
          properties.setProperty("failures", failures.toString())
          properties.setProperty("healthy", "false")
          writeSafetyProperties(properties)
          if (failures >= BOOT_FAILURE_LIMIT) {
            setEmergencyDisabled(true, "automatic safe mode after $failures system_server failures")
            true
          } else false
        }
        .onFailure { Log.e(TAG, "Failed to record system_server failure", it) }
        .getOrDefault(false)
  }

  private fun readSafetyProperties(): Properties =
      Properties().also { properties ->
        if (Files.exists(bootStatePath)) {
          runCatching { Files.newInputStream(bootStatePath).use { properties.load(it) } }
              .onFailure { Log.w(TAG, "Failed to read safety state", it) }
        }
      }

  private fun writeSafetyProperties(properties: Properties) {
    Files.newOutputStream(
            bootStatePath,
            StandardOpenOption.CREATE,
            StandardOpenOption.TRUNCATE_EXISTING,
            StandardOpenOption.WRITE)
        .use { properties.store(it, "Vector recovery state") }
    Os.chmod(bootStatePath.toString(), "600".toInt(8))
  }
