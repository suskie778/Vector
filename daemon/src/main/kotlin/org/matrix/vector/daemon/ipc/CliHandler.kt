...
object CliHandler {
  ...
  private fun handleStatus(): Map<String, Any> {
    return mapOf(
        ...
        "Emergency Disable" to FileSystem.isEmergencyDisabled(),
        "Status Notification" to PreferenceStore.isStatusNotificationEnabled())
  }

  private fun handleSafety(request: CliRequest): Any {
    return when (request.action) {
      "status" ->
          mapOf(
              "Emergency Disable" to FileSystem.isEmergencyDisabled(),
              "Disable Path" to FileSystem.emergencyDisablePath.toString())
      "enable" -> {
        check(FileSystem.setEmergencyDisabled(true, "cli")) {
          "Could not enable emergency disable"
        }
        "Emergency disable enabled. Reboot or restart affected processes to remove old hooks."
      }
      "disable" -> {
        check(FileSystem.setEmergencyDisabled(false)) {
          "Could not disable emergency disable"
        }
        "Emergency disable cleared. New processes may receive hooks again."
      }
      else -> throw IllegalArgumentException("Unknown safety action: ${request.action}")
    }
  }
