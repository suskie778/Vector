...
@Command(name = "safety", description = ["Control the pre-injection emergency circuit breaker"])
class SafetyCommand {
  @ParentCommand lateinit var parent: Cli

  @Command(name = "status", description = ["Show emergency disable state"])
  fun status(): Int =
      OutputFormatter.print(VectorIPC.transmit(CliRequest(command = "safety", action = "status")), parent.json)

  @Command(name = "enable", description = ["Block all new Vector injection"])
  fun enable(): Int =
      OutputFormatter.print(VectorIPC.transmit(CliRequest(command = "safety", action = "enable")), parent.json)

  @Command(name = "disable", description = ["Allow Vector injection again"])
  fun disable(): Int =
      OutputFormatter.print(VectorIPC.transmit(CliRequest(command = "safety", action = "disable")), parent.json)
}
